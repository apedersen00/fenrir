...
class FenrirNet(nn.Module):
    def __init__(self, config):
        ...
        self.fc1        = qnn.QuantLinear((h // 4) * (w // 4) * conv2_out, 64, bias=False, weight_bit_width=fc1_bits)
    ...
    def forward(self, x: torch.Tensor):
        ...
        for t in range(T):
            ...
            cur1 = self.fc1(spk2.view(B, -1))
            fc_mem1 = fnr.NetUtils.mem_clamp(fc_mem1, scale_fc1, multiplier=self.fc1_multiplier)
            spk3, fc_mem1 = self.lif1(cur1, fc_mem1)
            fc_mem1 = fnr.NetUtils.beta_clamp(fc_mem1, self.fc1_beta)

            cur2 = self.fc2(spk3)
            fc_mem2 = fnr.NetUtils.mem_clamp(fc_mem2, scale_fc2, multiplier=self.fc2_multiplier)
            spk4, fc_mem2 = self.lif2(cur2, fc_mem2)
            fc_mem2 = fnr.NetUtils.beta_clamp(fc_mem2, self.fc2_beta)
